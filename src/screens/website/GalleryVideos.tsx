...
import {
  websiteGalleryVideosAdd,
  websiteGalleryVideosDelete,
  websiteGalleryVideosList,
  websiteGalleryVideosReorder,
  websiteWorkingCopyStatus,
  type TourVideo,
} from "../../lib/website";

// Gallery videos editor (v3.23.0). Mirror of TourVideos but writes to
// gallery-videos.json, transcodes to assets/video/gallery/, and drafts
// under the gallery-videos revision key.
export default function GalleryVideos() {
  ...
  async function onUpload() {
    ...
    try {
      ...
      const res = await websiteGalleryVideosAdd(paths);
      setMsg(
  ...
  async function confirmDelete() {
    ...
    setErr(null);
    try {
      const rev = await websiteGalleryVideosDelete(pendingDelete.id);
  ...
  async function performBulkDelete() {
    ...
    try {
      ...
      for (const id of Array.from(selectedIds)) {
        const rev = await websiteGalleryVideosDelete(id);
        lastRev = rev;
      }
  ...
  async function commitReorder(next: TourVideo[]) {
    setItems(next);
    try {
      const rev = await websiteGalleryVideosReorder(next.map((v) => v.id));
  ...
  return (
    <div style={{ padding: 24, maxWidth: 1200, margin: "0 auto" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20 }}>
        ...
        <h1 style={{ margin: 0 }}>Gallery videos</h1>
        <span style={{ color: "var(--muted, #64748b)", fontSize: 13 }}>
          {items.length} video{items.length === 1 ? "" : "s"}
        </span>
        <div style={{ marginLeft: "auto", display: "flex", gap: 8 }}>
          <button className="btn" onClick={() => nav("/website/preview?page=gallery-videos")}>
            Preview →
          </button>
          <button
            className="btn"
            ...
            return (
              ...
              >
                ...
                <div style={{ padding: 12 }}>
                  ...
                  >
                    {v.id} · {v.src.replace("assets/video/gallery/", "")}
                  </div>
      ...
      >
        ...
        <a
          ...
          onClick={(e) => {
            e.preventDefault();
            nav("/website/edit/gallery-videos");
          }}
          style={{ color: "#6366f1" }}
        >
          Ask AI to update the Gallery Videos page
        </a>
        ...
        >
          ...
          >
            ...
            <p style={{ margin: "0 0 20px", color: "#475569" }}>
              <b>{pendingDelete.title}</b> will be removed from the Gallery Videos
              page and the underlying files will be deleted from your working
              copy. This creates a draft revision — the change goes live only
              after Publish.
            </p>
        ...
        >
          ...
          >
            ...
            <p style={{ margin: "0 0 16px", color: "#475569" }}>
              The following videos will be removed from the Gallery Videos page
              and their files deleted from your working copy. This creates a
              draft revision — the change goes live only after Publish.
            </p>
