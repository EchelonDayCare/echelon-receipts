...
If only one time is visible, set the other to null. Use {month}-01 numbering for the date prefix.",
    ...
    let text = resp.text().await.map_err(|e| format!("gemini read: {e}"))?;
    if !status.is_success() {
        return Err(format!("gemini http {status}: {}", truncate(&text, 800)));
    }
    ...
    let inner = v["candidates"][0]["content"]["parts"][0]["text"]
        .as_str()
        .ok_or_else(|| format!("gemini: no text in response: {}", truncate(&text, 400)))?
        .to_string();

    let parsed: serde_json::Value = serde_json::from_str(&inner)
        .map_err(|e| format!("model output not JSON: {e} :: {}", truncate(&inner, 400)))?;
